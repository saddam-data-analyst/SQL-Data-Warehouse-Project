...
BEGIN
	...
	BEGIN TRY
		...
		FROM (
		SELECT 
		--ROW_NUMBER()-Assign a unique number to each row in a result set, based on a defined order
		*,
		...
		SELECT
			...
			CAST(
				...
				AS DATE
			) AS prd_end_dt    --Calculate end date as on one day before the next start date
...
--Let's Executes the silver Procedure
EXEC silver.load_silver
